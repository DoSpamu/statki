...
// ─── Baner tury ───────────────────────────────────────────────────────────────
function TurnBanner({ isMyTurn }: { isMyTurn: boolean }) {
  return (
    <div className="w-full flex items-center justify-center gap-4 py-2">
      ...
      >
        {isMyTurn ? (
          <><span className="text-[#c8dc50]">⊕</span><span>TWOJA TURA — OTWÓRZ OGIEŃ</span></>
        ) : (
          <><span style={{ animation: 'bf-pulse 1s ease-in-out infinite' }}>◌</span><span>KOMPUTER MYŚLI…</span></>
        )}
      </div>
      <div className="flex-1 h-px" style={{ background: isMyTurn ? '#a8cc3044' : '#2a3a18' }} />
    </div>
...
function Divider({ isMyTurn, myHits, theirHits, total }: {
  isMyTurn: boolean; myHits: number; theirHits: number; total: number;
}) {
  return (
    <div className="flex flex-col items-center justify-center gap-4 pt-16 px-2 w-24">
      ...
      <div className="flex flex-col items-center gap-1">
        <span className="text-xl" style={{ color: isMyTurn ? '#c8dc50' : '#4a6a18' }}>
          {isMyTurn ? '⟶' : '⟵'}
        </span>
        <span className="text-[8px] tracking-widest uppercase" style={{ color: '#3a5818' }}>vs</span>
        <span className="text-xl" style={{ color: isMyTurn ? '#4a6a18' : '#c8dc50' }}>
          {isMyTurn ? '⟵' : '⟶'}
        </span>
      </div>
...
// ─── AIBattleView ─────────────────────────────────────────────────────────────
export default function AIBattleView({
  playerName, myGrid, myCellShipInfo, placedShips, applyIncomingShot, onReset,
}: AIBattleViewProps) {
  const {
    ...
    <>
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        <div className="flex items-start gap-0">
          {/* Moja plansza */}
          <div className="flex flex-col items-center gap-2">
            <BoardLabel>Moja flota — {playerName}</BoardLabel>
            <Board
              grid={myGrid}
              phase="battle"
          ...
          <Divider isMyTurn={isMyTurn} myHits={myHits} theirHits={theirHits} total={TOTAL} />

          {/* Plansza AI — statki ukryte */}
          <div className="flex flex-col items-center gap-2">
        ...
        <div className="flex gap-5 text-[9px] font-mono tracking-widest uppercase text-[#3a5818] mt-1">
          <span className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 inline-block bg-[#141d0c] border border-[#2a3a18]" />nieznany
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 inline-block bg-[#1e0400]" style={{ boxShadow: 'inset 0 0 5px #ff220018' }} />trafiony
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 inline-block bg-[#2a0000]" style={{ boxShadow: 'inset 0 0 5px #cc000033' }} />zatopiony
          </span>
          <span className="flex items-center gap-1.5">
